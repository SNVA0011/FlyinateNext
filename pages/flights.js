...
export default function Flights(pageValue, pageType) {
  ...
  <Col xs={12} md={6}>
                {/* <ScrollAnimation duration={1.2} animateOnce={true} animateIn='fadeInUp' animateOut='fadeOutDownBig' className='blogaddalist-round'> */}
                  <Link href={`/flights/${items.url}-${items.pageValue}`} className='image_area_partition'>
                    <a>
                  ...
                  <div className='blogaddalist-inner'>
                    ...
                    <Link href={`/flights/${items.url}`} className='h4 title'>
                     <span>{items.pageName}-{items.pageValue}</span>
                     </Link>
                    <hr className="mx-row-hr" />
                    <Link onClick={()=>SendingPageType(items)} href={`/flights/${items.url}-${items.pageValue}`} className='btn btn-site ripple-effbtn btn-40'>
                    <a>
                      <span>Read More</span>
                      </a>
                    </Link>
